/*
 * Author : recedivies
 * Problem Link : http://www.usaco.org/index.php?page=viewproblem2&cpid=691
 * Jun 24, 2021
 */
#include <bits/stdc++.h>
using namespace std; 
...
int main() {
 ...
 int ans = 0;
 vector<pair<int, int>> v {{0,1}, {0,2}, {1,0}, {1,2},{2,0},{2,1}};
 for (int opt = 0; opt < 6; opt++) {
  for (int i = 1; i <= n; i++) {
   ans = max(ans, a[i][v[opt].first] + a[n][v[opt].second] - a[i][v[opt].second]);
  }
 }
 cout << ans << "\n";
 return 0;
