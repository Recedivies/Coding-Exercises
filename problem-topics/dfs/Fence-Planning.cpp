/*
 * Author : recedivies
 * Problem Link : http://www.usaco.org/index.php?page=viewproblem2&cpid=944
 * Jun 14, 2021
 */
#include <bits/stdc++.h>
using namespace std; 
...
void dfs(int u, ll point[]) {
	vis[u] = 1;
	point[0] = min(point[0], p[u].first);
	point[1] = max(point[1], p[u].first);
	point[2] = min(point[2], p[u].second);
	point[3] = max(point[3], p[u].second);
	for (int v : edges[u]) {
		if (!vis[v]) {
			dfs(v, point);
		}
...
int main() {
	...
	for (int i = 1; i <= n; i++) {
		if (!vis[i]) {
			ll x1 = INT_MAX, x2 = 0, y1 = INT_MAX, y2 = 0;
			ll point[4] = {x1, x2, y1, y2};
			dfs(i, point);
			ans = min(ans, 2 * (point[1] - point[0] + point[3] - point[2]));
		}
	}
	cout << ans << '\n';
	return 0;
