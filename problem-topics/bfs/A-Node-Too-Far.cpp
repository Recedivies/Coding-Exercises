/*
 * Author : recedivies
 * Problem Link : https://onlinejudge.org/index.php?option=onlinejudge&Itemid=99999999&page=show_problem&category=0&problem=272&mosmsg=Submission+received+with+ID+26550344
 * Jul 07, 2021
 */
#include <bits/stdc++.h>
using namespace std; 
...
void generate() {
	for (int i = 1; i <= rep; i++) dist[i] = MAX;
}
...
int main() {
	...
	while (cin >> node) {
		...
		while (cin >> a >> b) {
			...
			cout << "Case " << ii++ << ": " <<  rep - counter - 1 << " nodes not reachable from node " << a << " with TTL = " << b << "." << '\n';
		}
		for (int i = 1; i <= rep; i++) {
			adj[i].clear();
		}
