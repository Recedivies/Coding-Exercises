/*
 * Author : recedivies
 * Problem Link : http://www.usaco.org/index.php?page=viewproblem2&cpid=620
 * Jun 04, 2021
 */
#include <bits/stdc++.h>
using namespace std; 
...
int main() {
	...
	q.push({0, {0, 0}}); // for k, {for x and y};
	int ans = MAX;
	...
	while (!q.empty()) {
		...
		q.pop();
		if (cnt == k) continue;
		if (!vis[i][j][0] && i > 0) { // empty 1
			vis[i][j][0] = 1;
			q.push({cnt+1, {0, j}});
		}
		if (!vis[i][j][1] && j > 0) { // empty 2
			vis[i][j][1] = 1;
			q.push({cnt+1, {i, 0}});
		}
		if (!vis[i][j][2] && i != x) { // fill 1
			vis[i][j][2] = 1;
			q.push({cnt+1, {x, j}});
		}
		if (!vis[i][j][3] && j != y) { // fill 2
			vis[i][j][3] = 1;
			q.push({cnt+1, {i, y}});
		}
		if (!vis[i][j][4] && i > 0 && j != y) { // pour 1 to 2
			vis[i][j][4] = 1;
			int rep = 0, ii = i, jj = j;
			while (ii != 0 && jj != y && rep != x) {
				rep++; ii--; jj++;
			}
			q.push({cnt+1, {ii, jj}});
		}
		if (!vis[i][j][5] && j > 0 && i != x) { // pour 2 to 1
			vis[i][j][5] = 1;
			int rep = 0, ii = i, jj = j;
			while (jj != 0 && ii != x && rep != y) {
				rep++; jj--; ii++;
			}
			q.push({cnt+1, {ii, jj}});
		}
