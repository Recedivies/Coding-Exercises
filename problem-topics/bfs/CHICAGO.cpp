/*
 * Author : recedivies
 * Problem Link : https://www.spoj.com/problems/CHICAGO/
 * Mei 23, 2021
 */
#include <bits/stdc++.h>
using namespace std; 
...
int main() {
	...
	while (cin >> n >> m) {
		...
		while (!q.empty()) {
			...
			for (pair<ld, ld> v : edges[u]) {
				if (dist[v.first] < 1.0 * dist[u] * v.second/100) {
					dist[v.first] = 1.0 * dist[u] * v.second/100;
					q.push({v.first});
				}
			}
		}
