...
int main() {
	...
	for (int i = 0; i < m; i++) {
		...
		cin >> ch >> u >> v;
		int tmp = (ch == "D" ? 1 : 0);
		edges[u].push_back({v, tmp});
		edges[v].push_back({u, tmp});
	}
	queue<int> q;
	int ans = 0;
	is_bipartite = 1;
		...
		if (vis[i] == -1) {
			...
			while (!q.empty()) {
				...
				for (pair<int, int> v : edges[u]) {
					if (vis[v.first] == -1) {
						if (v.second == 0) vis[v.first] = vis[u];
						else vis[v.first] = vis[u] ^ 1;
						q.push(v.first);
					} else {
						if (v.second == 0) is_bipartite &= vis[v.first] == vis[u];
						else is_bipartite &= vis[v.first] != vis[u];
					}
				}
			}
		}
	}
	if (!is_bipartite) cout << 0 << '\n';
	else {
		cout << 1;
