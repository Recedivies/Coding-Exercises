...
int main() {
	...
	cin>>ss>>tt;
	s += ss; t += tt;
	for (int i=1;i<sz(s);i++)dp[i][0]=i;
	for (int i=1;i<sz(t);i++)dp[0][i]=i;
	for (int i=1;i<sz(s);i++) for (int j=1;j<sz(t);j++){
		dp[i][j]=min({dp[i][j-1]+1, dp[i-1][j]+1, dp[i-1][j-1] + ((s[i]==t[j]) ^ 1)});
	}
	cout<<dp[sz(s)-1][sz(t)-1]<<'\n';
	return 0;
}
