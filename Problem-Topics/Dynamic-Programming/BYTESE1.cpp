/*
 * Author : recedivies
 * Problem Link : https://www.spoj.com/problems/BYTESE1/
 * Mei 22, 2021
 */
#include <bits/stdc++.h>
using namespace std; 
...
int n, m;
vector<vector<int>> a(N, vector<int>(N, 0)), dp(N, vector<int>(N, 0));
			...
			for (int i = 0; i < 4; i++) {
				...
				if (isValid(r, c) && dp[r][c] > dp[u][v]) {
					...
					for (int j = 0; j < 4; j++) {
						...
						if (isValid(rr, cc)) {
							dp[r][c] = min(dp[r][c], dp[rr][cc] + a[r][c]);
						}
					}
				}
			}
