/*
 * Author : recedivies
 * Problem Link : https://tlx.toki.id/courses/competitive/chapters/13/problems/F
 * Jul 04, 2021
 */
#include <bits/stdc++.h>
using namespace std; 
...
int prf[69][69], prfb[69][69], a[69][69], r[69][69];
...
int main() {
	...
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j <= m; j++) {
			...
			prf[i][j] = a[i][j] + prf[i - 1][j] + prf[i][j - 1] - prf[i - 1][j - 1];
			prfb[i][j] = r[i][j] + prfb[i - 1][j] + prfb[i][j - 1] - prfb[i - 1][j - 1];
		}
	}
	int ans = 0, rock = 0;
	for (int i = 1; i <= n; i++) { // a
		for (int j = 1; j <= m; j++) { // b
			for (int k = i; k <= n; k++) { // A
				for (int l = j; l <= m; l++) { // B
					int sum = prf[k][l] - prf[i - 1][l] - prf[k][j - 1] + prf[i - 1][j - 1];
					int sumb = prfb[k][l] - prfb[i - 1][l] - prfb[k][j - 1] + prfb[i - 1][j - 1];
