	...
	while (cin >> n >> c && (n && c)) {
		...
		for (int i = 0; i < c; i++) {
			...
			if (m[{u, v}] == 1 || m[{v, u}] == 1) {
				...
				m[{u, v}] = 1; m[{v, u}] = 1; // need map to ensure no multiple edges bcs (i assume that possible multiple edges in input with constraint <= 5000 whereas the node only <= 100)
				edges[u].push_back(v);
				edges[v].push_back(u);
			}
		}
