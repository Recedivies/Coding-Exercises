...
int x1, x2, yy1, y2, px1, px2, py1, py2;

bool isValid() {
	return !((x1 < px1 && px1 < x2) || (yy1 < py2 && py2 < y2));
}
...
int main() {
	...
	cin >> x1 >> yy1 >> x2 >> y2;
	int area1 = abs(x2 - x1) * abs(y2 - yy1);
	
	cin >> px1 >> py1 >> px2 >> py2;
	
	if ((px1 <= x1 && px2 >= x2 && isValid()) || (py1 <= yy1 && py2 >= y2 && isValid())) {
		int area2 = ( abs(max({px1, x1}) - min({px2, x2})) ) * ( abs(max({py1, yy1}) - min({py2, y2})) );
		cout << area1 - area2 << '\n';
	}
	else cout << area1 << '\n';
	return 0;
