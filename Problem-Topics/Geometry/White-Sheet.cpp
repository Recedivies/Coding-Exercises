...
ll intersect3(Rect p, Rect q, Rect r) {
	ll xOverlap = max(0LL, min(min(p.x2, q.x2),r.x2) - max(max(p.x1, q.x1),r.x1));
	ll yOverlap = max(0LL, min(min(p.y2, q.y2),r.y2) - max(max(p.y1, q.y1),r.y1));
	return xOverlap * yOverlap; 
}
...
int main() {
	...
	ll wb1b2 = intersect3(w, b1, b2);
	if (wArea > wb1 + wb2 - wb1b2) cout << "YES";
	else cout << "NO";
	return 0;
}
