/*
 * Author : recedivies
 * Problem Link : https://tlx.toki.id/problems/bnpchs-2009-final/E
 * Jun 10, 2021
 */
#include <bits/stdc++.h>
using namespace std; 
...
bool vis[N + 1][N + 1]; 
int valid[N + 1][N + 1];
vector<pair<int, int>> p(N*N + 1), bom(N*N + 1);
char a[N + 1][N + 1], ch;
...
int main() {
	...
	while (tt--) {
		...
		scanf("%d", &b);
		for (int i = 1; i <= b; i++) {
			scanf("%d %d %d %d", &p[i].first, &p[i].second, &bom[i].first, &bom[i].second);
			valid[p[i].first][p[i].second] = i;
		}
		bool ok = 0;
		while (!q.empty()) {
			...
			for (int k = 0; k < 4; k++) {
				...
				if (isValid(r, c) && !vis[r][c] && a[r][c] != '#') {
					vis[r][c] = 1;
					q.push({r, c});
					if (valid[r][c] != -1) {
						int id = valid[r][c];
						if (a[bom[id].first][bom[id].second] == '#') {
							a[bom[id].first][bom[id].second] = '.';
							vis[bom[id].first][bom[id].second] = 1;
							q.push({bom[id].first, bom[id].second});
						}
					}
				}
			}
