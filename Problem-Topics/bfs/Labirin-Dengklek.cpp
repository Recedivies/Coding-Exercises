...
int main() {
		...
		for (int i = 0 ; i < n; i++) {
			for (int j = 0; j < m; j++) {
				...
				for (int rep = 0; rep < 4; rep++) {
					int r = dx[rep] + i;
					int c = dy[rep] + j;
					if (isValid(r, c) && dp1[r][c][k - 1] && a[i][j] == s[k]) {
						ok = 1;
 					}
				}
				dp1[i][j][k] |= ok;
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				...
				for (int rep = 0; rep < 4; rep++) {
					int r = dx[rep] + i;
					int c = dy[rep] + j;
					if (isValid(r, c) && dp2[r][c][k + 1] && a[i][j] == t[k]) {
						ok = 1; 
					}
				}
				dp2[i][j][k] |= ok;
	...
	for (int k = 0; k < ls; k++) {
		int x = ls - k;
		if (x > lt) continue;
		if (s.substr(k, x) != t.substr(0, x)) continue;
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				if (dp1[i][j][k] && dp2[i][j][0]) {
					cout << k + lt << '\n';
					return 0;
				}
			}
	...
	while (!q.empty()) {
		...
		for (int k = 0; k < 4; k++) {
			...
			if (isValid(r, c) && dist[r][c] > dist[i][j] + 1) {
				dist[r][c] = dist[i][j] + 1;
				q.push({r, c});
			}
		}
	}
	cout << (ans == -1 ? ans : ans + ls + lt) << '\n';
	return 0;
}
