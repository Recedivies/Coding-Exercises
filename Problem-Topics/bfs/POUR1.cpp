...
int main() {
	...
	while (tt--) {
		...
		int ans = -1;
		while (!q.empty()) {
			int cnt = q.front().first, p1 = q.front().second.first, p2 = q.front().second.second;
			q.pop();
			...
			int pourA = min(p1, b - p2);
			int pourB = min(p2, a - p1);
			int na[6] = {p1, 0, a, p1, p1 - pourA, p1 + pourB};
			int nb[6] = {0, p2, p2, b, p2 + pourA, p2 - pourB};
			for (int i = 0; i < 6; i++) {
				if (m[{na[i], nb[i]}] == 0) {
					m[{na[i], nb[i]}] = 1;
					q.push({cnt+1, {na[i], nb[i]}});
				}
			}
		}
		cout << ans << '\n';
